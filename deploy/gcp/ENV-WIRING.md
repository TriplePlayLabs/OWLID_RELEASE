...
For project `owlid-491411` (number `922474514728`) in `europe-west1`:

| Service | URL |
|---------|-----|
| `verification` | `https://verification-922474514728.europe-west1.run.app` |
| `issuer` | `https://issuer-922474514728.europe-west1.run.app` |
| `sidecar` | `https://sidecar-922474514728.europe-west1.run.app` |
| `app` | `https://app-922474514728.europe-west1.run.app` |
| `admin` | `https://admin-922474514728.europe-west1.run.app` |
| `verifier` | `https://verifier-922474514728.europe-west1.run.app` |

Predictability matters — no chicken-and-egg dependency between services. Defined as `local.run_url` in `terraform/locals.tf`.

When you wire the custom domain (`*.owlid.app`), swap these for the user-facing URLs.
...
| `MIDNIGHT_ENABLED` | TF static `false` | flip to `true` after wiring real Midnight infra |
| `MIDNIGHT_SIDECAR_URL` | TF `local.run_url["sidecar"]` | sidecar Cloud Run URL |
...
| `MIDNIGHT_ENABLED` | TF static `false` | |
| `MIDNIGHT_SIDECAR_URL` | TF `local.run_url["sidecar"]` | |
...
### `app`, `admin`, `verifier` (static SPA → nginx)

Frontends do **not** read env vars at process level. `docker/runtime-config.sh` runs at container start, reads `OWLID_*` env vars, and writes `/config.js` with `window.__OWLID_CONFIG__ = { ... }`. The SDK reads from there at runtime.
...
| `verifier` | `OWLID_API_KEY` | secret `api-key-dev` |

The verifier app is the only frontend that needs an API key — it is a public-facing tool that calls the verification service directly. Other frontends rely on the user's own admin session JWT.
...
| Real Midnight chain | `MIDNIGHT_NODE_WS_URL`, `MIDNIGHT_INDEXER_URI`, `MIDNIGHT_INDEXER_WS_URI`, `MIDNIGHT_PROOF_SERVER_URI` | Edit `terraform/run.tf::sidecar` env block, replace placeholders with real URLs. Flip `MIDNIGHT_ENABLED=true` on verification + issuer. |
